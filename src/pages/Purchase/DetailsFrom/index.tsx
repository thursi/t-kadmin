    ...
    <div className="w-full flex-col py-2">
      <div className="flex justify-between mb-1">
        ...
        <div className="flex-1 grid grid-cols-4 gap-2">
          ...
          <div>
            <strong className="text-sm">Purchase Date :</strong>{" "}
            {props.item?.purchaseDate || "--"}
          </div>
          <div>
            <strong className="text-sm">Purchase Status :</strong>
            {props.item?.purchaseStatus === "ORDERED" && (
              <span className="text-success bg-[#E7F7EF] px-3 py-1.5 rounded-full">
                {props.item?.purchaseStatus}
              </span>
            )}
            {props.item?.purchaseStatus === "RECEIVED" && (
              <span className="text-primary bg-[#E0F7FF] px-3 py-1.5 rounded-full">
                {props.item?.purchaseStatus}
              </span>
            )}
            {props.item?.purchaseStatus === "PENDING" && (
          ...
          <div>
            <strong className="text-sm">Total Items :</strong>{" "}
      ...
      <div className="w-full mt-4 border p-4">
        ...
        <table className="min-w-full border-collapse">
          ...
          <tbody>
            {Array.isArray(props.item?.productPurchaseResponses) ? (
              props?.item?.productPurchaseResponses.map(
                (purchase: any, index: any) => {
                  const product = purchase.productVariableResponse;
